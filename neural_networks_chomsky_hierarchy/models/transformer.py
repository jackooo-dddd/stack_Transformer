...
class TransformerConfig:
  ...
  positional_encodings_params: pos_encs_lib.PositionalEncodingsParams = None
...
def shift_right(x: chex.Array, output_size: int) -> chex.Array:
  ...
  """
  ## shift_right is designed for teacher forcing in the autoregressive tasks.
  We feed the corrct output at the previous timestamps into the model as the input at curr timestamp.
  E.g:The target is [A, B, C], right shift it produces  [<START>, A, B], serving as input.
  """
...
def compute_sliding_window_mask(sequence_length: int,
  ...
  ## For sliding window masking attention.
  """
...
class Stack(hk.Module):
  def __init__(self) -> None:
    super().__init__()
    self.action = hk.Linear(3)

  def __call__(self, h : chex.Array) -> chex.Array:
    return self.stack(h)
  
  def stack(self, h : chex.Array) -> chex.Array:
    batch_size, sequence_length, _ = h.shape
    stacks = jnp.zeros((batch_size, sequence_length+1, sequence_length))
    stacks = stacks.at[:,0,0].set(1)
    actions = jnn.softmax(self.action(h), axis=-1) ## Compute the action weights,  aᵢ = softmax(Wₐhᵢ + bₐ)
    ## For each time step i from 0 to sequence_length - 1:
    for i in range(sequence_length):
      """
    ## Turn the three “action scores” at position i into three matrices of shape [sequence_length]:
      push_action[t] tells us how much we want to push on step i for depth t,
      pop_action[t] tells us how much we want to pop on step i for depth t,
      no_op_action[t] tells us how much we want to stay put for depth t.
      """
      tiled_stack_actions = einshape.jax_einshape(
          'ba->bsa', actions[:,i,:], s=sequence_length)
      push_action = tiled_stack_actions[..., 0]
      pop_action = tiled_stack_actions[..., 1]
      no_op_action = tiled_stack_actions[..., 2]

      ## Build the “push value” vector: If we push at i, the new top-of-stack depth is exactly i.
      push_value = jnp.zeros((batch_size, sequence_length))
      push_value = push_value.at[:, i].set(1)
      ## noop leaves the stack unchanged.
      no_op_value = stacks[:, i, :]
      ## Pop value: take everything from the previous stack but shift it down one level.
      ## Look at all depths < i, and mix them with the full stack, dropping the current top.
      pop_value = jnp.einsum('bt,btd->bd', no_op_value[:, :i], stacks[:, :i, :]) if i > 0 else no_op_value
      ## combine these three possibilities into the new stack state at time i+1
      new_stack = (
          push_action * push_value + pop_action * pop_value +
          no_op_action * no_op_value)
      stacks = stacks.at[:, i+1, :].set(new_stack)
    ## Reading the top the stack.
    top_stacks = jnp.einsum('btT,bTd->btd', stacks[:,1:,:], h)
    return top_stacks
...
class TransformerEncoder(hk.Module):
  ...
  def __call__(self, x: jnp.ndarray) -> chex.Array:
    ...
    for _ in range(self._config.num_layers):
      ...
      if self._config.feedforward:
        feedforward = hk.Linear(self._config.embedding_dim * self._config.widening_factor)(
            h)
        feedforward = jnn.relu(feedforward)
        feedforward = hk.Linear(self._config.embedding_dim)(feedforward)

        feedforward = hk.dropout(hk.next_rng_key(), self._config.dropout_prob, feedforward)
        feedforward = layer_norm(feedforward + h)
        h = feedforward
      ## After the usual Attention→LayerNorm and Feed‑Forward→→LayerNorm steps, the model calls Stack attention.
      ## Stack module computes for each position a learned push/pop/no‑op sequence and returns Stack Top of same shape as h.
      if self._config.stack:
        top_stacks = Stack()(h)
        ## Transformer’s hidden representations are augmented with the outcome from the stack.
        h = top_stacks + h
    return h
...
class TransformerDecoderOnly(hk.Module):
  """Transformer Decoder (Vaswani et al., 2017)."""
    ...
    for _ in range(self._config.num_layers):
      if self._config.attention:
        self_attention = MultiHeadDotProductAttention(
            ...
            positional_encodings_params=self._config.positional_encodings_params,
            attention_window=self._config.attention_window,
        )(inputs_q=h, inputs_kv=h, mask=causal_mask, causal=self._config.causal_masking)
        self_attention = hk.dropout(hk.next_rng_key(), self._config.dropout_prob,
                                    self_attention)
        self_attention = layer_norm(h + self_attention)
        h = self_attention
      ...
      if self._config.stack:
        top_stacks = Stack()(h)
        # top_stacks = hk.dropout(hk.next_rng_key(), self._config.dropout_prob, top_stacks)
        # top_stacks = layer_norm(top_stacks + h)
        h = top_stacks + h
...
def make_transformer_decoder(
  ...
  def transformer_decoder(inputs: chex.Array) -> chex.Array:
    output = TransformerDecoderOnly(config)(inputs)
    if not return_all_outputs:
      output = output[:, -1, :]
    return hk.Linear(output_size)(output)
