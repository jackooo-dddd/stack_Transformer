...
class TrainingWorker:
  ...
  def run(
    ...
    """
    ## Training params are passed from the example.py
    ...
    ## JAX/Haiku random‑number generator sequence (rng_seq) we’ll draw from later
    ...
    if training_params.is_autoregressive:
      ## Autoregressive model expects both inputs and outputs at initialization time.
      params = model.init(
          model_init_rng_key,
          dummy_batch["input"],
          dummy_batch["output"],
    ...
    opt_state = optimizer.init(params) # Initializing the optimizer state
    ...
    for step in steps:
      ...
      ## Create examples of given length. The type of task is from the input parameters
    ...
    if training_params.compute_full_range_test:
      eval_params = range_evaluation.EvaluationParams(
          ...
          max_test_length=training_params.max_range_test_length, ## =100
    ...
    """
    results: logged training‑loop metrics (Training accuracy over time)
    eval_results: full‑range test accuracies (Testing accuracy)
    params: the final trained parameters
    """
